...
class ModelS9(nn.Module):
    def __init__(self):
        # super is used to initialize the parent class (nn.Module) so that we can use the
        # methods and members of the parent class in Model1 class
        super(ModelS9, self).__init__()
        self.kernel_size = (3, 3)
        self.dropout_value = 0.05
        ...
        self.inputblock = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=16, kernel_size=self.kernel_size, padding=1, stride=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(16),
          	nn.Dropout(self.dropout_value)
        )
        ...
        #CONVBLOCK 1
        self.convblock1 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=self.kernel_size, padding=1, stride=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(32),
          	nn.Dropout(self.dropout_value)
        )
        ...
        self.convblock2 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=self.kernel_size, padding=1, stride=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(32),
          	nn.Dropout(self.dropout_value)
        )
        self.convblock3 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=128, kernel_size=self.kernel_size, padding=1, dilation=2, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(128),
          	nn.Dropout(self.dropout_value)
        )
        ...
        self.depthwise1 = nn.Sequential(nn.Conv2d(in_channels=128, out_channels=128, kernel_size=self.kernel_size, stride=1, padding=1, groups=32),
                        nn.ReLU(),
                        nn.BatchNorm2d(128),
                        nn.Dropout(self.dropout_value)
                        )
        self.pointwise1 = nn.Sequential(nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1),
                        nn.ReLU(),
                        nn.BatchNorm2d(64),
                        nn.Dropout(self.dropout_value)
                        )

        self.convblock4 = nn.Sequential(self.depthwise1, self.pointwise1)

        self.convblock5 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=self.kernel_size, padding=1, stride=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(64),
          	nn.Dropout(self.dropout_value)
        )
        self.convblock6 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=32, kernel_size=self.kernel_size, padding=1, dilation=2, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(32),
          	nn.Dropout(self.dropout_value)
        )
        ...
        self.depthwise2 = nn.Sequential(nn.Conv2d(in_channels=32, out_channels=32, kernel_size=self.kernel_size, stride=1, padding=1, groups=32),
                        nn.ReLU(),
                        nn.BatchNorm2d(32),
                        nn.Dropout(self.dropout_value)
                        )
        self.pointwise2 = nn.Sequential(nn.Conv2d(in_channels=32, out_channels=64, kernel_size=1),
                        nn.ReLU(),
                        nn.BatchNorm2d(64),
                        nn.Dropout(self.dropout_value)
                        )

        self.convblock7 = nn.Sequential(self.depthwise2, self.pointwise2)
        ...
        self.convblock9 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=16, kernel_size=self.kernel_size, padding=1,dilation=2, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(16),
          	nn.Dropout(self.dropout_value)
        )
        ...
        #OUTPUT BLOCK
        self.convblock10 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=16, kernel_size=self.kernel_size, bias=False)
            )
